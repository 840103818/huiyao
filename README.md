...
当前版本：`2.0.3` · Bundle ID：`com.huiyao.studio` · 支持 Apple Silicon 与 macOS 12 及以上版本。
...
从 [GitHub Releases](https://github.com/840103818/huiyao/releases/latest) 下载 `Huiyao_2.0.3_aarch64.dmg`，打开后将“绘钥”拖入“应用程序”。
...
```bash
shasum -a 256 -c Huiyao_2.0.3_aarch64.dmg.sha256
```
...
3. 启动队列。分析期间顶部工具栏只保留一个“停止生成”入口；停止会取消全部活动请求，等待工作线程退出，并把未完成任务标记为暂停。
4. 单项失败不会阻塞后续任务，可在任务菜单中重试、重命名、移动或删除。

停止后的“继续队列”会重新发起模型请求，不从中断的 SSE 位置续传。已完成任务的“重新分析”默认更新原任务，只有任务菜单中的“创建关联副本”会增加任务。
...
artifacts/release/绘钥_2.0.3_aarch64.dmg
```
