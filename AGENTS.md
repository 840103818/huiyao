...
- `PromptVersion`、EXIF 白名单和结果导出结构变更必须同时核对前端浏览器降级序列化与 Rust 原生导出。
- Vite 配置以 `vite.config.ts` 为唯一来源；不要保留会遮蔽它的生成型 `vite.config.js`。
...
- 每次代码修改同步更新相关用户操作说明和技术说明，不让文档落后于实现。
- 页面布局或样式修改后覆盖更新 `docs/assets/ui/current/` 中的双尺寸、双主题和关键交互截图。
- 截图只使用测试数据，不留存 API Key、模型正文、用户原图或其他敏感信息。
