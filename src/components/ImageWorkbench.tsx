...
import { setViewerChromeHidden } from "../lib/bridge";
...
export function ImageWorkbench({
  ...
  const clampStagePan = useCallback((value: { x: number; y: number }, atZoom = zoom) => {
    ...
    if (!stage || !imageInfo || atZoom <= 100) return { x: 0, y: 0 };
  ...
  const handleStageKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (event.key === "Enter") {
      ...
      event.preventDefault();
      resetStage();
    } else if (zoom > 100 && ["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(event.key)) {
      event.preventDefault();
    ...
    <section ref={panelRef} className="input-lab panel" aria-busy={loading}>
      ...
      <div className="image-workspace">
        <div className="image-stage-wrap">
          <div
            ref={stageRef}
            className={`image-stage ${zoom > 100 ? "can-pan" : ""} ${loading ? "is-processing" : ""}`}
          ...
          >
            ...
            {displayImage && !parametersOpen ? (
              <div className="image-floating-tools" role="toolbar" aria-label="图片画布工具">
                ...
                <ToolButton label={fitMode === "contain" ? "填满画布" : "适应画布"} onClick={() => onFitModeChange(fitMode === "contain" ? "cover" : "contain")}><IconExpand /></ToolButton>
