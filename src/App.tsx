...
const LogsView = lazy(() => import("./components/LogsView").then((module) => ({ default: module.LogsView })));
...
export default function App() {
  ...
  useEffect(() => {
    const handlePaste = (event: ClipboardEvent) => {
      ...
      event.preventDefault();
      if (loading) {
        showNotice("生成或图片处理中不能替换图片", "error");
        return;
      }
      ...
      const apply = () => void handleImageFile(file);
  ...
  }, [activeHistoryId, handleImageFile, loading, result, showNotice, view]);
...
function countCompletedAnalysis(result: ReverseResult | null): number {
  if (!result) return 0;
...
function clipboardTimestamp(): string {
  const date = new Date();
